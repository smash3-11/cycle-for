...
for (let i = 1; i <= tries; i++) {
    let user = +prompt(`Введите пароль, у вас  ${tries - i} попытки:`);
    if (user === password) {
...
for (let i = 0; i < users.length; i++) {
    let user = users[i];
    let login = user.email.split('.').pop();
    if (login in emails) {
        emails[login].push(user);
    } else {
...
//     } else if (user.email.endsWith('.net')) {
//       emails.net.push(user);
//     } else if (user.email.endsWith('.info')) {
